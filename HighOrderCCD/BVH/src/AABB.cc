...
namespace aabb
{
    ...
    bool AABB::overlaps(const AABB& aabb, bool touchIsOverlap, double margin) const
    {
        assert(aabb.lowerBound.size() == lowerBound.size());
        ...
        if (touchIsOverlap)
        {
            for (unsigned int i = 0; i < lowerBound.size(); ++i)
            {
                if (aabb.upperBound[i] + margin < lowerBound[i] || aabb.lowerBound[i] > upperBound[i] + margin)
                {
                    rv = false;
                    break;
                }
    ...
    std::vector<unsigned int> Tree::query(unsigned int particle, double margin)
    {
        ...
        return query(particle, nodes[particleMap.find(particle)->second].aabb, margin);
    }

    std::vector<unsigned int> Tree::query(unsigned int particle, const AABB& aabb, double margin)
    {
        ...
        while (stack.size() > 0)
        {
            ...
            if (aabb.overlaps(nodeAABB, touchIsOverlap, margin))
            {
    ...
    std::vector<std::pair<unsigned int,unsigned int>> Tree::query(const Tree& _tree, double margin)
    {
        std::vector<std::pair<unsigned int,unsigned int>> stack;
        stack.reserve(256);
        stack.push_back(std::make_pair(root,_tree.root));

        std::vector<std::pair<unsigned int,unsigned int>> particles;

        while (stack.size() > 0)
        {
            std::pair<unsigned int,unsigned int> node_pair = stack.back();
            unsigned int node = node_pair.first;
            unsigned int _node = node_pair.second;
            stack.pop_back();

            // Copy the AABB.
            AABB nodeAABB = nodes[node].aabb;
            AABB _nodeAABB = _tree.nodes[_node].aabb;

            if (node == NULL_NODE) continue;
            if (_node == NULL_NODE) continue;
            ...
            if (isPeriodic)
            {
                ...
                std::vector<double> shift(dimension);
                for (unsigned int i=0;i<dimension;i++)
                    separation[i] = nodeAABB.centre[i] - _nodeAABB.centre[i];
            ...
            */
            ...
            // Test for overlap between the AABBs.
            if (_nodeAABB.overlaps(nodeAABB, touchIsOverlap, margin))
            {
                // Check that we're at a leaf node.
                if (nodes[node].isLeaf() && _tree.nodes[_node].isLeaf())
                {
                    // Can't interact with itself.
                    //if (nodes[node].particle != particle)
                    //{
                    /*
                    std::cout<<"id:"<<nodes[node].particle<<" "<<_tree.nodes[_node].particle<<std::endl;
                    std::cout<<"lower:"<<nodeAABB.lowerBound[0]<<" "<<nodeAABB.lowerBound[1]<<" "<<nodeAABB.lowerBound[2]<<std::endl;
                    std::cout<<"upper:"<<nodeAABB.upperBound[0]<<" "<<nodeAABB.upperBound[1]<<" "<<nodeAABB.upperBound[2]<<std::endl;
                    std::cout<<"lower:"<<_nodeAABB.lowerBound[0]<<" "<<_nodeAABB.lowerBound[1]<<" "<<_nodeAABB.lowerBound[2]<<std::endl;
                    std::cout<<"upper:"<<_nodeAABB.upperBound[0]<<" "<<_nodeAABB.upperBound[1]<<" "<<_nodeAABB.upperBound[2]<<std::endl;
                    */
                    particles.push_back(std::make_pair(nodes[node].particle, _tree.nodes[_node].particle));
                    //}
                }
                else
                {
                    if (nodes[node].isLeaf())
                    {
                       stack.push_back(std::make_pair(node,_tree.nodes[_node].left));
                       stack.push_back(std::make_pair(node,_tree.nodes[_node].right));
                    }
                    else if(_tree.nodes[_node].isLeaf())
                    {
                       stack.push_back(std::make_pair(nodes[node].left,_node));
                       stack.push_back(std::make_pair(nodes[node].right,_node));
                    }
                    else
                    {
                       stack.push_back(std::make_pair(nodes[node].left,_tree.nodes[_node].left));
                       stack.push_back(std::make_pair(nodes[node].right,_tree.nodes[_node].right));
                       
                       stack.push_back(std::make_pair(nodes[node].left,_tree.nodes[_node].right));
                       stack.push_back(std::make_pair(nodes[node].right,_tree.nodes[_node].left));
                    }
    ...
    std::vector<unsigned int> Tree::query(const AABB& aabb, double margin)
    {
        ...
        return query(std::numeric_limits<unsigned int>::max(), aabb, margin);
    }
