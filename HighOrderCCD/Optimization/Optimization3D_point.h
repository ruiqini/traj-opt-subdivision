...
public:
 ...
 //Eigen::Dynamic
                           ...
                           BVH& bvh)
  {
      ...
      if(max_step<1e-10)
      {
        Subdivide::update_tree_point(spline,V,bvh);
      }
      if(adaptive_change)
      {
        Subdivide::update_tree_point(spline,V,bvh);
      ...
      */
      ...
      clock_t time0 = clock();
      descent_direction(spline, direction,piece_time, t_direction, V,bvh);
      clock_t time1 = clock();     
      
      line_search(spline, direction,piece_time, t_direction, V, bvh, false);//vtxs
                                ...
                                BVH& bvh)
  {
    ...
    clock_t time1 = clock();
    Gradient::fast_point_barrier_gradient(spline, g2,h2, V,bvh);
    clock_t time2 = clock();
                          ...
                          BVH& bvh, bool exact)
  {
    ...
    double step=Step::point_position_step(spline, direction,V, bvh);
    ...
    // backtracking
    //time0 = clock();
    double e=Energy::fast_point_whole_energy(spline,piece_time,V,bvh);
    double init_time=piece_time;
   
    piece_time=init_time+step*t_direction;
    while(e-1e-4*wolfe*step<Energy::fast_point_whole_energy(spline+step*direction,piece_time,V,bvh))
    {
      step*=0.8;
      piece_time=init_time+step*t_direction;
    }
    ...
    std::cout<<"result:"<<Energy::dynamic_energy(spline+step*direction,piece_time)<<
               " "<<lambda*Energy::point_barrier_energy(spline+step*direction,V,bvh)<<
